...
export default function Approvals() {
  ...
  const approveMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest("POST", `/api/submissions/${id}/approve`, {
        status,
        comment,
      });
    },
    onSuccess: () => {
      ...
      if (actionType === "reject") message = "Submission rejected.";
      if (actionType === "escalate")
        message = "Submission escalated to HR/Admin.";
    ...
    onError: (error: Error) => {
      ...
      toast({
        title: "Error",
        description: `Failed to ${actionType} submission. Please try again.`,
        variant: "destructive",
      });
    },
  });
  ...
  const escalateMutation = useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      return apiRequest("POST", `/api/submissions/${id}/escalate`, { reason });
    },
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Submission escalated to HR/Admin.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/approvals/pending"] });
      setSelectedSubmission(null);
      setComment("");
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: "Failed to escalate submission. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const confirmAction = () => {
    ...
    if (actionType === "escalate" && !comment.trim()) {
      toast({
